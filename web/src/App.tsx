  ...
  return (
    <div className={`min-h-screen bg-neutral-950 text-white role-${role.replace(/\s+/g,'').toLowerCase()}`}>
      <div className="max-w-7xl mx-auto px-6 py-4 space-y-4">
        ...
        {tab==="Operate" && <Operate role={role} runId={runId} projectId={project?.id} onQueue={(item)=>{ const it = project? { ...item, projectId: project.id } : item; setQueue(q=>[...q, it]); }} onEvent={(msg)=>setLastEvent(msg)} />} 
